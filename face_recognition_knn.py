...
def draw_preds(img_path, preds, filesta):
    """
    ...
    :return:
    """
    basedir = "/home/deeplearning/nasshare/chitack.chang/face/whowho"
    prefix = basename(img_path).split("_")[0]
    prefix2 = basename(img_path).split("_who")[0]
    if not filesta.get(prefix): filesta[prefix] = {}
    ...
    for pred in preds:
        ...
        if not filesta[prefix].get(name):
            ...
            filesta[prefix][name] = filesta[prefix][name] + 1
...
if __name__ == "__main__":
    ...
    #testfilefolder = "knn_examples/test"
    testfilefolder = "/home/deeplearning/Desktop/face_recognition examples/output"

    filesta = {}
    totalcount = len(listdir(testfilefolder))
    index = 1
    fulllist = listdir(testfilefolder)
    fulllist.sort()
    for img_path in fulllist:
        print("%d/%5d" % (index, totalcount))
        index = index + 1
        preds = predict(join(testfilefolder, img_path) ,knn_clf=knn_clf)
        #print(preds)
        draw_preds(join(testfilefolder, img_path), preds, filesta)

    #print(filesta)
    #sorted(filesta)
    for fn in sorted(filesta):
